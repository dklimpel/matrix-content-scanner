...
function generateReportHash(
  ...
  thumbnailQueryParams = undefined
) {
  ...
  return base64sha256(
    JSON.stringify({ httpUrl, matrixFile, thumbnailQueryParams })
  );
}
...
const getReport = async function(console, domain, mediaId, matrixFile, opts) {
  ...
  if (!reportCache[reportHash]) {
    console.info(
      `File not scanned yet: domain = ${domain}, mediaId = ${mediaId}`
    );
  ...
  console.info(
    `Returning scan report: domain = ${domain}, mediaId = ${mediaId}, clean = ${clean}`
  );
...
function getInputHash(_, domain, mediaId, matrixFile, opts) {
  ...
  const httpUrl = generateHttpUrl(
    ...
    domain,
    mediaId,
    Boolean(opts.thumbnailQueryParams)
  );
...
function deduplicatePromises(getKey, asyncFn) {
  ...
  return async (console, ...args) => {
    ...
    if (!ongoing[k]) {
      ongoing[k] = asyncFn(console, ...args).finally(res => {
        delete ongoing[k];
        return res;
      });
...
const generateReportFromDownload = deduplicatePromises(
  getInputHash,
  _generateReportFromDownload
);
...
async function _generateReportFromDownload(
  ...
  try {
    ...
    console.error(
      `Receieved status code ${err.statusCode} when requesting ${httpUrl}`
    );

    throw new ClientError(
      502,
      "Failed to get requested URL",
  ...
  console.info(
    `Result: url = "${httpUrl}", clean = ${result.clean}, exit code = ${
      result.exitCode
    }`
  );
...
async function generateReport(
  ...
  if (matrixFile && matrixFile.key) {
    ...
    try {
      ...
      throw new ClientError(
        400,
        "Failed to copy file for decryption",
        "MCS_MEDIA_FAILED_TO_DECRYPT"
      );
    }
  }
